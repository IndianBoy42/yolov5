...
def init():
    ...
    imgsz_detect = check_img_size(
        imgsz_detect, s=model_detect.stride.max()
    )  # check img_size
    imgsz_recog = check_img_size(
        imgsz_recog, s=model_recog.stride.max()
    )  # check img_size
    ...
    names_detect = (
        model_detect.module.names
        if hasattr(model_detect, "module")
        else model_detect.names
    )
    names_recog = (
        model_recog.module.names
        if hasattr(model_recog, "module")
        else model_recog.names
    )
...
def proc(img, im0s, view_img=False, save_img=False):
    ...
    for i, det in enumerate(pred):  # detections per image
        ...
        if len(det):
            ...
            img_lp0s = [
                extract_img_lp0(im0, xyxy, img_lp, device, imgsz_recog, half)
                for *xyxy, _, _ in reversed(det)
            ]
            img_lps = [
                extract_img_lp1(img_lp0, img_lp, device, imgsz_recog, half)
                for img_lp0 in img_lp0s
            ]

            height = max([x.shape[-2] for x in img_lps])
            width = max([x.shape[-1] for x in img_lps])
            img_lps = [
                torch.nn.ZeroPad2d((0, width - x.shape[-1], 0, height - x.shape[-2]))(x)
                for x in img_lps
            ]
            ...
            # Apply NMS
            pred_lps = non_max_suppression(
                infs,
                conf_thres_recog,
                iou_thres_recog,
                classes=classes_recog,
                agnostic=agnostic_nms,
            )
            ...
            # Write results
            for (*xyxy, conf, cls), img_lp, img_lp0, pred_lp in zip(
                reversed(det), img_lps, img_lp0s, pred_lps
            ):
                ...
                s += license_str + " " + ("%g " * len(line)).rstrip() % line + "\n"
